...
public final class TreeUtilUser {

	public static void resetRootUser(Element root) {
	...
	public static void createHtmlUser(Element root, StringBuffer html, String id) {
		List<Element> elements = root.elements();
		if (elements.size() > 0) {
	...
	public static void createOrgHtml(Element root, StringBuffer html, int level) {
		...
		if (elements.size() > 0) {
			...
			for (Element child : elements) {
				...
				if (type.equals("org")) {
					html
							.append("</td><td><a href=\"../../System/OrgManage/edit?id=");
					html.append(id);
					html
							.append("\"><img src=\"../../img/edit.gif\"/></a><a href=\"#\" onClick=\"submitSingleDelete(");
				} else if (type.equals("user")) {
					html
							.append("</td><td><a href=\"../../System/UserManage/edit?id=");
					html.append(id);
					html
							.append("\"><img src=\"../../img/edit.gif\"/></a><a href=\"#\" onClick=\"submitSingleDelete2(");
				}
				html.append(id);
				html
						.append(", this)\"><img src=\"../../img/del.gif\"/></a></td></tr>");
	...
	public static void createDocTypeHtml(Element root, StringBuffer html,
		...
		if (elements.size() > 0) {
			...
			for (Element child : elements) {
				...
				html.append("\"");
				html
						.append("><td align=\"center\"><input type=\"checkbox\" name=\"idList\" value=\"");
				html.append(id);
				html
						.append("\" attach=\"selectAll\" /></td><td style=\"text-align:left\">");
				for (int i = 0; i < level; i++)
					html.append("&nbsp;&nbsp;&nbsp;&nbsp;");
				if (child.elements().isEmpty()) {
					html.append(child.attributeValue("name"));
				...
				html
						.append("</td><td><a href=\"../../Flow/DocTypeManage/edit?id=");
				html.append(id);
				html
						.append("\"><img src=\"../../img/edit.gif\"/></a><a href=\"#\" onClick=\"submitSingleDelete(");
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if(childElement.elements().size()==0){
					 ...
					 */
					html.append("<a href=");
					html.append(childElement.attributeValue("link"));    //  超链接 URL  地址
				...
				for (int j = 0; j < childListOne.size(); j++) {
					...
					for (int k = 0; k < childListTwo.size(); k++) {
					  ...
					  html.append("<div id=\"pNode"+i+"_sub"+j+"_s"+k+"\">");				  
					  /**  					
					    *  只做到三级菜单没有判断 是否有 第四级菜单
					    */
					  
					  html.append("<a href=");
					  html.append(childOne.attributeValue("link"));    //  超链接 URL  地址
